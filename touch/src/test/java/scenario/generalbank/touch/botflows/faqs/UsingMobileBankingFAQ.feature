Feature: General Bank FAQ: "How long must I wait until I can use Mobile Banking?" intent
  ...
  Scenario Outline: Verify if user receives answer on "<user input>" message
    ...
    Examples:
      ...
      |How long must I wait until I can use Mobile Banking? | There is no wait at all. Once you have successfully registered at one of our branches you can start using Mobile Banking immediately.|