...
public class IntegrationSteps extends GeneralSteps {
    ...
    public void getIntegrationConfiguration(Map<String, String> dataMap) {
        ...
        } else if (getResponseCode(response) == 404) {
            validateErrorResponse(response, dataMap);
        } else {
            Assertions.fail(format("The response code is not as expected %n expected: %s ,%n actual %s", getExpectedCode(dataMap), getResponseCode(response)));
        }
    }
