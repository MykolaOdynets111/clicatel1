Feature: General Bank FAQ: "How much does General Bank MasterPass app cost?" intent
  ...
  Scenario Outline: Verify if user receives answer on "<user input>" message
    ...
    Examples:
      ...
      |How much does General Bank MasterPass app cost?  |Hi ${firstName}. There are no fees or charges to clients to use the MasterPass app.|
      |Need some help or assistance with the General Bank MasterPass app?|Hi ${firstName}. Contact us on our 24hr channels: T 0860 10 20 43 F +27 21 941 0770 E ClientCare@Generalbank.co.za|