Feature: VM flow regarding Quote
  ...
  Scenario Outline: "Quote" flow should work for "<user message>" user message
    ...
    Then User have to receive 'Hi ${firstName}. To apply for a credit card online, click on the link below and then click 'apply now'. https://www.virginmoney.co.za/credit-card/' text response for his '<user message>' input
    Examples:
      |user message                     |
