Feature: VM flow regarding Blocked Card
  ...
  Scenario Outline: "Blocked Card" flow should work for "<user message>" user message
    ...
    Then User have to receive 'Hi ${firstName}. Your security is very important to us. We will block a card from doing further transactions if our fraud department notices suspicious activity on the card. Many of these blocks happen automatically, so you may try and do a transaction before we can tell you about the block.' text response with Blocked card intent for his '<user message>' input
    Examples:
      |user message                                                                               |
