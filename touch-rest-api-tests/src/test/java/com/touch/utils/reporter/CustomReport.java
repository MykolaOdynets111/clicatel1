package com.touch.utils.reporter;
...
public class CustomReport implements IReporter {
    ...
    protected void writeStylesheet() {
        ...
        writer.print("table {margin-bottom:5px;border-collapse:collapse;empty-cells:show}");
    ...
    private int writeScenarioSummary(String description, List<ClassResult> classResults,
        ...
        if (!classResults.isEmpty()) {
            ...
            for (ClassResult classResult : classResults) {
                ...
                buffer.setLength(0);
                int scenariosPerClass = 1;
                for (MethodResult methodResult : classResult.getMethodResults()) {
                    int resultsCount = methodResult.getResults().size();
                    for (ITestResult testResult : methodResult.getResults()) {
                        buffer.append("<tr class=\"" + cssClass + "\">");
                        long start = testResult.getStartMillis();
                        long duration = testResult.getEndMillis() - start;
                        buffer.append("<td>" + Utils.escapeHtml(testResult.getMethod().getMethodName()) + "</td>");
                        if (testResult.getParameters().length > 0) {
                            buffer.append("<td>" + Arrays.toString(testResult.getParameters()) + "</td>");
                        } else {
                            buffer.append("<td></td>");
                        }
                        buffer.append("<td>").append(parseUnixTimeToTimeOfDay(start)).append("</td>")
                                .append("<td class=\"numi\">").append(decimalFormat.format(millisecondsToSeconds(duration))).append("</td></tr>");
                    }
                    scenariosPerClass += resultsCount;
                }
                // Write the test results for the class
                writer.print("<tr class=\"" + cssClass + "\">");
                writer.print("<td rowspan=\"" + scenariosPerClass + "\">");
