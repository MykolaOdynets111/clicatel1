...
public class OpenedTweet extends AbstractUIElement {
    ...
    public void sendReply(String tweetText, String agentMessage){
        new TweetsSection(this.getCurrentDriver()).sendReplyForTweet(agentMessage, tweetText);
//        moveToElemAndClick(replyInputArea);
//        findElemByCSS(replyInputFieldCSS).sendKeys(tweetText);
        executeJSclick(findElemByXPATH(this.getCurrentDriver(), replyButtonOnTweetComment), this.getCurrentDriver());
    }
    ...
    public void closeTweet(){
        findElemByCSS(this.getCurrentDriver(), closeTweetButtonCSS).click();
    }

    public boolean ifAgentReplyShown(String replyText, int wait){
        try {
            waitForElementToBeVisibleByXpath(this.getCurrentDriver(), String.format(agentResponseXpath, replyText.trim()),wait);
            return true;
